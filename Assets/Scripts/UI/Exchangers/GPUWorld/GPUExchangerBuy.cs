...
public class GPUExchangerBuy : MonoBehaviour
{
    private List<GPUExchangerItem> toBuy = new List<GPUExchangerItem>();
    public GPUWorldItemSO so;
    public OWInvSO data;
    ...
    public void SelectItem(GPUExchangerItem item)
    {
    ...
    private void SelectNewOrDeselectPrevious(GPUExchangerItem item)
    {
    ...
    private async void HandleThePurchase()
    {
        if (toBuy.Count > 0)
        {
            GPUExchangerItem GPUItem = toBuy[0];
            if (GPUItem != null)
            {
                //you can place the condition for currency here
                int index = GPUItem.temporaryIndex;
                GPUs sp = GetItemAt(index);
    ...
    //}
    public void GPUBuys(GPUExchangerItem GPUItem)
    {
        if (GPUItem != null)
        {
            Debug.Log("Buyedd");

            Parts = ConvertGPUtoCollect(GPUItem);
    ...
    public PartsCollect ConvertGPUtoCollect(GPUExchangerItem shopItem)
    {

        List<GPUs> shopItems = so.gpus;
        int tempIndexs;
        int originalIndex;
        int tempIndex;
        ...
        if (tempToOriginalIndexMapping.TryGetValue(tempIndex, out int originalIndexs))
        {
            ...
            GPUs shpItem = GetItemAt(originalIndexs);
