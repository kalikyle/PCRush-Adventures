...
public class PSUWorldExchangerBuy : MonoBehaviour
{
    private List<PSUWorldExchangerItem> toBuy = new List<PSUWorldExchangerItem>();
    public PSUWorldItemSO so;
    public OWInvSO data;
    ...
    public void SelectItem(PSUWorldExchangerItem item)
    {
    ...
    private void SelectNewOrDeselectPrevious(PSUWorldExchangerItem item)
    {
    ...
    private async void HandleThePurchase()
    {
        if (toBuy.Count > 0)
        {
            PSUWorldExchangerItem PSUItem = toBuy[0];
            if (PSUItem != null)
            {
                //you can place the condition for currency here
                int index = PSUItem.temporaryIndex;
                PSUs sp = GetItemAt(index);
    ...
    //}
    public void PSUBuys(PSUWorldExchangerItem PSUItem)
    {
        if (PSUItem != null)
        {
            Debug.Log("Buyedd");

            Parts = ConvertPSUtoCollect(PSUItem);
    ...
    public PartsCollect ConvertPSUtoCollect(PSUWorldExchangerItem shopItem)
    {

        List<PSUs> shopItems = so.psus;
        int tempIndexs;
        int originalIndex;
        int tempIndex;
        ...
        if (tempToOriginalIndexMapping.TryGetValue(tempIndex, out int originalIndexs))
        {
            ...
            PSUs shpItem = GetItemAt(originalIndexs);
