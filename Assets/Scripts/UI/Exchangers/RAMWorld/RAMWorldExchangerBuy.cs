...
public class RAMWorldExchangerBuy : MonoBehaviour
{
    private List<RAMWorldExchangerItem> toBuy = new List<RAMWorldExchangerItem>();
    public RAMWorldItemSO so;
    public OWInvSO data;
    ...
    public void SelectItem(RAMWorldExchangerItem item)
    {
    ...
    private void SelectNewOrDeselectPrevious(RAMWorldExchangerItem item)
    {
    ...
    private async void HandleThePurchase()
    {
        if (toBuy.Count > 0)
        {
            RAMWorldExchangerItem RAMItem = toBuy[0];
            if (RAMItem != null)
            {
                //you can place the condition for currency here
                int index = RAMItem.temporaryIndex;
                RAMs sp = GetItemAt(index);
    ...
    //}
    public void RAMBuys(RAMWorldExchangerItem RAMItem)
    {
        if (RAMItem != null)
        {
            Debug.Log("Buyedd");

            Parts = ConvertRAMtoCollect(RAMItem);
    ...
    public PartsCollect ConvertRAMtoCollect(RAMWorldExchangerItem shopItem)
    {
        ...
        if (tempToOriginalIndexMapping.TryGetValue(tempIndex, out int originalIndexs))
        {
            ...
            RAMs shpItem = GetItemAt(originalIndexs);
