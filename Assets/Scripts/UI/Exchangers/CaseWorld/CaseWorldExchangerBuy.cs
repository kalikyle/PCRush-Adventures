...
public class CaseWorldExchangerBuy : MonoBehaviour
{
    private List<CaseWorldExchangerItem> toBuy = new List<CaseWorldExchangerItem>();
    public CaseWorldItemSO so;
    public OWInvSO data;
    ...
    public void SelectItem(CaseWorldExchangerItem item)
    {
    ...
    private void SelectNewOrDeselectPrevious(CaseWorldExchangerItem item)
    {
    ...
    private async void HandleThePurchase()
    {
        if (toBuy.Count > 0)
        {
            CaseWorldExchangerItem CaseItem = toBuy[0];
            if (CaseItem != null)
            {
                //you can place the condition for currency here
                int index = CaseItem.temporaryIndex;
                Cases sp = GetItemAt(index);
    ...
    //}
    public void CaseBuys(CaseWorldExchangerItem CaseItem)
    {
        if (CaseItem != null)
        {
            Debug.Log("Buyedd");

            Parts = ConvertCasetoCollect(CaseItem);
    ...
    public PartsCollect ConvertCasetoCollect(CaseWorldExchangerItem shopItem)
    {

        List<Cases> shopItems = so.cases;
        int tempIndexs;
        int originalIndex;
        int tempIndex;
        ...
        if (tempToOriginalIndexMapping.TryGetValue(tempIndex, out int originalIndexs))
        {
            ...
            Cases shpItem = GetItemAt(originalIndexs);
